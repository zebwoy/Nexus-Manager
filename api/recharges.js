...
export default async function handler(req, res) {
  ...
  return withTenantClient(pool, req, res, async (client) => {
    // ─── Route: PATCH /api/recharges/:id ──────────────────────────
    const idMatch = subPath.match(/^(\d+)(\?|$)/) || rawUrl.match(/\/recharges\/(\d+)(\?|$)/)
    if (idMatch) {
      const rcId = Number(idMatch[1])
    ...
    if (req.method === 'POST') {
      const b = req.body || {}
      const { customer_id, name, mobile, game_platform, cost_price, charge_price, payment_received, note, date } = b
      ...
      try {
        ...
        const r = await client.query(
          `INSERT INTO recharges (customer_id, game_platform, cost_price, charge_price, payment_received, note, date, created_by)
          ...
          [
            ...
            Number(cost_price),
            Number(charge_price),
            payment_received !== undefined && payment_received !== null ? Number(payment_received) : null,
            note || null,
