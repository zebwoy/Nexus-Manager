...
export default function Sessions() {
  const [activeTab, setActiveTab] = useState('grid') // 'grid' | 'entries'
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.75rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 className="page-title">
            {activeTab === 'grid' ? 'Gaming Sessions' : 'Session Entries'}
          </h1>
          <p className="page-sub">
            {activeTab === 'grid'
              ? 'Station floor matrix and real-time terminal status'
              : 'Historical gameplay transaction logs and audit trail'}
          </p>
        </div>
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          {activeTab === 'grid' ? (
            ...
            <button
              ...
              style={{ padding: '0.6rem 1.25rem', display: 'flex', alignItems: 'center', gap: '0.45rem' }}
            >
              <LayoutGrid size={14} /> ← Station Floor Matrix
            </button>
          )}
          <button
      ...
      {/* VIEW 1: LIVE VISUAL STATION FLOOR MATRIX */}
      {activeTab === 'grid' && (
        <StationGrid
      ...
      {/* VIEW 2: FULL-WIDTH EXPANSIVE SESSION ENTRIES LEDGER */}
      {activeTab === 'entries' && (
        loading ? <PageLoader /> : sessions.length === 0 ? (
          <EmptyState
            title="No Sessions Logged"
            description={`No gaming session logs recorded for date: ${formatDate(dateFilter)}`}
              ...
              <tbody>
                {sessions.map((s, index) => {
                  ...
                  return (
                    <tr
                      key={s.id}
                      style={{ cursor: 'pointer', background: index % 2 === 0 ? 'rgba(0,0,0,0.015)' : 'transparent' }}
